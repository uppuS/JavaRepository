...
public class TranslatorTest {
	private NumberToWord numberToWordTranslator = new NumberToWord();
    ...
    public void shouldThrownAnExceptionIfTooLargeANumberIsPassedIn() {
    	numberToWordTranslator.translate(1000000000);
    }


    private void shouldTranslate(int[] from, String[] to) {
        for (int i = 0; i < from.length; i++) {
            assertEquals(to[i], numberToWordTranslator.translate(from[i]));
        }
    }
